...
export default function JiraCard({ configStatus, onStatusChange }) {
  ...
  const handleVerify = async () => {
    ...
    try {
      const r = await fetch('/api/config/verify-jira', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jiraUrl, jiraEmail, jiraToken }) });
      const data = await r.json(); setResult(data);
      if (data.ok) {
        await fetch('/api/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jiraUrl, jiraEmail, jiraToken }) });
